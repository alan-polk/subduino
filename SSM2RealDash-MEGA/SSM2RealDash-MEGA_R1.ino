...
int OILPin = 8; // Oil Pressure on Pin 8
...
uint8_t pollECUPacket[63] = {
  ...
  0x3F, // payload size  /// must be updated if you add or remove parameters
...
void setup() {
  ...
  Serial1.begin(9600); // for BT to Dash
...
// Get new SSM Data every 20 milliseconds
  if (Time - LastRequest > 20) {
    ...
    if(serialCallSSM(kLine, pollECUPacket, RequestPacketSize, ECUBuffer, ResponsePacketSize, 0) < 10)
   {
      LastRequest = millis();
